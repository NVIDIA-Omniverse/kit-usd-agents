...
@echo off
REM Windows script to run Isaac Sim MCP Server locally
REM This script starts the server for local development

echo ========================================
echo Isaac Sim MCP Server - Local Development
echo ========================================
echo.
...
set ISAACSIM_MCP_DISABLE_USAGE_LOGGING=false
set MCP_PORT=9904

REM Display startup information
echo Starting Isaac Sim MCP Server...
echo Port: %MCP_PORT%
...
if exist "workflows\local_config.yaml" (
    echo Config: workflows\local_config.yaml
    poetry run isaacsim-mcp workflows\local_config.yaml
) else (
    echo Note: Using config.yaml as local_config.yaml not found
    echo Config: workflows\config.yaml
    poetry run isaacsim-mcp workflows\config.yaml
)
