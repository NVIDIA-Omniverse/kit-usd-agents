...
# Unix script to run Kit MCP Server locally
# This script starts the server for local development
set -e  # Exit on any error

echo "========================================"
echo "Kit MCP Server - Local Development"
echo "========================================"
echo
...
export KIT_MCP_DISABLE_USAGE_LOGGING=false
...
if [ -f "workflows/local_config.yaml" ]; then
    poetry run kit-mcp workflows/local_config.yaml
else
    echo "Note: Using config.yaml as local_config.yaml not found"
    poetry run kit-mcp workflows/config.yaml
fi