...
def _get_or_create_reranker(reranking_config: Optional[Dict[str, Any]] = None):
    ...
    if _reranker is None or reranking_config:
        ...
        if _reranker:
            logger.info("Reranker initialized for Kit test examples")
...
async def search_test_examples(
    ...
    of Kit test examples. It uses FAISS vector similarity search with NVIDIA embeddings, followed by
    optional reranking for improved relevance.

    How it works:
    1. Converts your query to embeddings using NVIDIA's nv-embedqa-e5-v5 model
    2. Performs semantic similarity search against pre-indexed Kit test examples
    3. Optionally reranks results using NVIDIA's llama-nemotron-rerank-1b-v2 model
    4. Returns formatted test examples with their metadata

    Args:
        query: Test scenario or functionality to find examples for
    ...
    Returns:
        ...
        - result: Formatted test examples with setup, execution, and validation patterns
    ...
    try:
        ...
        if not code_search_service.is_available():
            error_msg = "Test search data is not available"
        ...
        search_results = code_search_service.search_test_examples(
