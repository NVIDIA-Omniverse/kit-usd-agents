...
from isaacsim_fns.functions.search_code_examples import get_code_search_service, search_code_examples
from isaacsim_fns.services.code_search_service import CodeSearchService
...
async def test_valid_query():
    ...
    with patch("isaacsim_fns.functions.search_code_examples.get_code_search_service") as mock_service:
        ...
        # Mock telemetry
        with patch("isaacsim_fns.functions.search_code_examples.ensure_telemetry_initialized", new_callable=AsyncMock):
            with patch("isaacsim_fns.functions.search_code_examples.telemetry") as mock_telemetry:
...
@pytest.mark.asyncio
async def test_search_timeout_returns_error():
    """If the blocking search exceeds the async timeout, return a clear error (no hang)."""
    with patch("isaacsim_fns.functions.search_code_examples.get_code_search_service") as mock_service:
        mock_instance = MagicMock()
        mock_instance.is_available.return_value = True
        mock_service.return_value = mock_instance
        ...
        with patch("isaacsim_fns.functions.search_code_examples.ensure_telemetry_initialized", new_callable=AsyncMock):
            with patch("isaacsim_fns.functions.search_code_examples.telemetry") as mock_telemetry:
                mock_telemetry.capture_call = AsyncMock()
                with patch(
                    "isaacsim_fns.functions.search_code_examples.asyncio.wait_for",
...
async def test_python_code_validation():
    ...
    # Test queries - use common Isaac Sim operations that should be in the database
    test_queries = [
        "How to create a robot?",
        "How to setup physics simulation?",
    ...
    for query in test_queries:
        ...
        result = await search_code_examples(query, top_k=3)
