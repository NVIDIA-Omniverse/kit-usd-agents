...
class GetIsaacSimExtensionDetailsInput(BaseModel):
...
class GetIsaacSimExtensionDetailsConfig(FunctionBaseConfig, name="get_isaac_sim_extension_details"):
    ...
    name: str = "get_isaac_sim_extension_details"
    verbose: bool = Field(default=False, description="Enable detailed logging")


@register_function(config_type=GetIsaacSimExtensionDetailsConfig, framework_wrappers=[])
async def register_get_isaac_sim_extension_details(config: GetIsaacSimExtensionDetailsConfig, builder: Builder):
    """Register get_extension_details function with AIQ."""

    verbose = config.verbose

    if verbose:
        logger.info(f"Registering get_isaac_sim_extension_details in verbose mode")

    async def get_isaac_sim_extension_details_wrapper(input: GetIsaacSimExtensionDetailsInput) -> str:
        ...
        try:
            ...
            if verbose:
                if isinstance(extension_ids_to_fetch, list):
                    logger.debug(f"Retrieved details for {len(extension_ids_to_fetch)} Isaac Sim extensions")
            ...
            if usage_logger and usage_logger.enabled:
                try:
                    ...
                    usage_logger.log_tool_call(
                        tool_name="get_isaac_sim_extension_details",
    ...
    function_info = FunctionInfo.from_fn(
        get_isaac_sim_extension_details_wrapper,
        description=GET_ISAAC_SIM_EXTENSION_DETAILS_DESCRIPTION,
        input_schema=GetIsaacSimExtensionDetailsInput,
    )
