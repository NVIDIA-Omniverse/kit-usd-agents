...
class SearchIsaacSimSettingsInput(BaseModel):
    """Input for search_settings function."""

    query: str = Field(description="Search query for finding relevant Isaac Sim configuration settings")
...
class SearchIsaacSimSettingsConfig(FunctionBaseConfig, name="search_isaac_sim_settings"):
    ...
    name: str = "search_isaac_sim_settings"
    verbose: bool = Field(default=False, description="Enable detailed logging")


@register_function(config_type=SearchIsaacSimSettingsConfig, framework_wrappers=[])
async def register_search_isaac_sim_settings(config: SearchIsaacSimSettingsConfig, builder: Builder):
    ...
    if verbose:
        logger.info(f"Registering search_isaac_sim_settings in verbose mode")

    async def search_isaac_sim_settings_wrapper(input: SearchIsaacSimSettingsInput) -> str:
        """Search for Isaac Sim configuration settings using semantic search."""
        import time
            ...
            if usage_logger and usage_logger.enabled:
                try:
                    ...
                    usage_logger.log_tool_call(
                        tool_name="search_isaac_sim_settings",
    ...
    function_info = FunctionInfo.from_fn(
        search_isaac_sim_settings_wrapper,
        description=SEARCH_ISAAC_SIM_SETTINGS_DESCRIPTION,
        input_schema=SearchIsaacSimSettingsInput,
    )
